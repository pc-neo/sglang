...
class ModelRunner:
    ...
    def _forward_raw(
        ...
        if can_run_cuda_graph:
            now = time.time()
            logger.info(f"run_cuda_graph replay start_time{now}")
            ...
            logger.info(f"run_decode use time: {time.time() - now}")
        elif forward_batch.forward_mode.is_extend():
            now = time.time()
            logger.info(f"run_extend start_time{now}")
            ret = self.forward_extend(
                forward_batch,
                skip_attn_backend_init=skip_attn_backend_init,
                pp_proxy_tensors=pp_proxy_tensors,
            )
            logger.info(f"run_extend use time: {time.time() - now}")
